...
data_file_replaced_location="${data_file_replaced}_with_url.json"
data_file_replaced_matched="${data_file_replaced}_matched.json"
uuid_file="data/uuids.txt"
data_dir="data"
records_dir="data/records"

# Step 1: Search and replace values in .supplementaryDocs[].url field
...
# Step 2: Move the value from url to location
jq 'map(.supplementaryDocs |= map(if .url != null then .location = .url | .url = null else . end))' \
    "$data_file_replaced" > "${data_file_replaced_location}"
data_file_replaced="${data_file_replaced_location}"
echo "Step 2: Moved url to location. Updated file: $data_file_replaced"
